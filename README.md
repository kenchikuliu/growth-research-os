...
Also supports a public difficulty hint from `https://seo.web.cafe/kd/`, exposed through `scripts/web_cafe_kd.py` and optional `kd_score / kd_input` workflow fields. The workflow now auto-fetches live KD by default through the page-token + `api/kd` path, with manual score and prebuilt JSON as fallback.
...
- `Similarweb` still has partial gaps for the deepest landing-pages style report automation, but it now emits structured `keyword_research` and `landing_pages_research` layers on top of `网站表现` / website-performance, `网站内容`, `搜索概况`, quick-search seeds, route candidates, and priority alerts, plus `readiness / row_counts / evidence_summary` fields so later scale logic can tell “only seeds” from “deeper rows reached”.
...
- `web_cafe_kd.py` now auto-fetches live structured JSON from `seo.web.cafe/kd/` through the homepage `kd-token` plus the site-local `api/kd` endpoint. It does not replace Semrush/Similarweb, but it helps decide whether to attack a head term directly or cut narrower into alternative / scenario / template pages.
- `web_cafe_kd.py` still accepts `--kd-score` and `--kd-input` as fallback modes when live fetch is disabled or unavailable.
...
- `workflow_verdict.py` is the next-layer packager over `keyword_verdict` and `page_artifacts`. It now emits `page_artifact_input` for verdict-driven page generation and `high_level_scale_verdict` for scale / batch consumption, including a `business_snapshot` and `business_template` block so `榜单归因 / 新词验证` can feed batch boards and reviewer-facing outputs directly.
